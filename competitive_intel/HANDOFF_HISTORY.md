# Competitive Intelligence System — Handoff History

Archived from `HANDOFF.md` on 2026-09-24. **The live doc is [`HANDOFF.md`](HANDOFF.md); start there.**

This file holds the dated sections from 2026-06-01 to 2026-07-22, covering the original build and launch. It is kept for the reasoning behind decisions made then, not because anything here is outstanding. Everything still live when this was archived was promoted into `HANDOFF.md` first: the newsletter audience membership, the Teams chat participants question, the `NEWSLETTER_RECIPIENTS` dead constant, the `TEAMS_GENERAL_WEBHOOK` secret-scoping decision, the repo path note, and the Resend `segment_id` gotcha.

Sections below are verbatim and in their original order.
